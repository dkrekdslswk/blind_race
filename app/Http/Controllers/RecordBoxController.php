                    ...
                    case 'root':
                        ...
                        $raceData = DB::table('raceUsers as ru')
                            ->select(
                                ...
                                DB::raw('count(CASE WHEN qb.type = "vocabulary obj" AND qb.type = "vocabulary sub" THEN 1 END) as vocabularyCount'),
                                DB::raw('count(CASE WHEN qb.type like "vocabulary%" AND re.answerCheck = "O" THEN 1 END) as vocabularyRightAnswerCount'),
                                DB::raw('count(CASE WHEN qb.type = "word obj" AND qb.type = "word sub" THEN 1 END) as wordCount'),
                                DB::raw('count(CASE WHEN qb.type like "word%" AND re.answerCheck = "O" THEN 1 END) as wordRightAnswerCount'),
                                DB::raw('count(CASE WHEN qb.type = "grammar obj" AND qb.type = "grammar sub" THEN 1 END) as grammarCount'),
